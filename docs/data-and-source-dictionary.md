...
| Evidence records           |      21 |
| Sources                    |      35 |
| Sources marked primary     |      33 |
| Sources marked non-primary |       2 |

The evidence cutoff remains 3 September 2026. A targeted review on 11 September added two previously omitted sources from within that window. Each source retains its actual access date.
...
| `accessed`            | ISO date | Baseline review date; individual source access dates are authoritative         |
| `reviewedOn`          | ISO date | Most recent targeted editorial/research review, not a new comprehensive cutoff |
...
Version 0.2.0 source categories:
...
`public/data/evidence.csv` has one row per evidence record. All cells are quoted, embedded quotation marks are doubled, and arrays are joined with semicolons. Formula-leading cells are prefixed with an apostrophe for safer spreadsheet import.

The evidence library also offers a filtered CSV export. It contains only visible records, identifies the source organization, and includes provenance and caveat fields. The footer downloads always contain the full dataset.
